...
- [x] `[BE]` 포인트 사용 시 500 `INTERNAL_ERROR` (2026-09-11) — 바로 위 멱등성 키 기능 자체의 버그. `PointRepositoryImpl.registerUseRequestIfAbsent()`(`REQUIRES_NEW`)가 UNIQUE 위반(`DataIntegrityViolationException`)을 메서드 내부에서 잡아 `false`를 반환하면, 이미 Postgres에 의해 abort된 이 트랜잭션을 Spring이 커밋 시도하다 `UnexpectedRollbackException`을 던짐 — 동일 `idempotencyKey` 재요청(더블클릭·새로고침 재제출 등) 시 잔액/원장 상태와 무관하게 100% 재현. 예외를 메서드 밖으로 그대로 던져 정상 롤백되게 하고, 호출자 `PointService.use()`에서 잡아 기존과 동일하게 처리하도록 수정. `PointServiceIdempotencyTest`의 mock 계약을 `false` 반환 → 예외로 갱신해 검증. Render 프로덕션 로그(`UnexpectedRollbackException` 스택트레이스)로 원인 확인
