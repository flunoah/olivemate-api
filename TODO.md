...
- [ ] `[BE]` `PointLedgerJpaRepository.findBalanceAggregates()`가 네이티브 SQL이라 `point_ledger` 테이블/컬럼명(`crew_id`, `ledger_type`, `remaining`, `amount`, `granted_at`, `expired_at`)이 문자열로 하드코딩됨 — 컬럼명 마이그레이션 시 컴파일러가 못 잡고 호출 시점에야 500. (`LedgerType` enum 값은 리터럴 대신 `PointRepositoryImpl`에서 `.name()`으로 넘기도록 고쳐서 enum 리네임은 컴파일 에러로 걸리게 해결됨.) 테이블/컬럼명 리팩터링 시 이 쿼리 같이 확인할 것
